...
void scale_bias_gpu(CLArray output, CLArray biases, int batch, int n, int size)
{
	...
	for (int i = 0; i < batch; i++)
	{
        ...
        cl_int error = clEnqueueNDRangeKernel(*cl->queue, kernel, 3, offset, global_size, NULL, 0, NULL, &events[i]);
		cl->checkError(error);
	}
...
void backward_scale_gpu(CLArray x_norm, CLArray delta, int batch, int n, int size, CLArray scale_updates)
{
	...
	cl_event e;
    cl_int error = clEnqueueNDRangeKernel(*cl->queue, kernel, 2, NULL, global_size, group_size, 0, NULL, &e);
	cl->checkError(error);
...
void add_bias_gpu(CLArray output, CLArray biases, int batch, int n, int size)
{
	...
	cl_event e;
    cl_int error = clEnqueueNDRangeKernel(*cl->queue, kernel, 3, NULL, global_size, NULL, 0, NULL, &e);
	cl->checkError(error);
	cl->checkError(clWaitForEvents(1, &e));
	clReleaseEvent(e);
}
...
void supp_gpu(int N, float ALPHA, CLArray X, int INCX)
{
	...
	size_t offset[] = { 0,0,0 };
	cl_event e;
    cl_int error = clEnqueueNDRangeKernel(*cl->queue, kernel, 3, offset, global_size, NULL, 0, NULL, &e);
	cl->checkError(error);
...
void smooth_l1_gpu(int n, CLArray pred, CLArray truth, CLArray delta, CLArray error)
{
	...
	cl_event e;
    cl_int status = clEnqueueNDRangeKernel(*cl->queue, kernel, 3, NULL, global_size, NULL, 0, NULL, &e);
